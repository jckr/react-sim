...
We add a little Reset button at the end so users can reset the data.
...
We can see time, we can move the slider back and forth, though it doesn't do anything.
...
Setting the `maxTime` to `Infinity` is the simplest way to make the slider go away.
If there's no upper bound to the value of time, then the slider is no longer displayed.
